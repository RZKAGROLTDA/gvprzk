# Etapa 2 (revisada) — "Meu Dia": ajuste apenas de `open_tasks`

Única mudança em relação ao SQL aprovado anteriormente: a regra do bloco `open_tasks` (agora "Próximas Ações").
Metas, visitas realizadas, ligações/prospecções, programação, retornos, treinamentos, segurança, permissões, estrutura do JSON e paginação permanecem idênticos.

Nova regra do bloco:

```text
fonte de pendência : next_action_date IS NOT NULL
exclusão           : COALESCE(status,'pending') NOT IN ('closed','completed')
overdue            : next_action_date < hoje
today              : next_action_date = hoje
upcoming           : next_action_date > hoje AND next_action_date <= hoje + 7
```

`start_date` e `end_date` não são mais usados para classificar. Continuam apenas como campos informativos do item.

## 1. SQL final completo

```sql
-- =========================================================
-- HELPER INTERNO (inalterado)
-- =========================================================
...
BEGIN
  ...
  -- ---------- LIGAÇÕES/PROSPECÇÕES REALIZADAS ----------
  ...
  -- ---------- PRÓXIMAS AÇÕES (chave técnica: open_tasks) — REGRA NOVA ----------
  WITH a AS (
...
-- =========================================================
-- RPC DE DETALHES (paginada) — só o ramo open_tasks mudou
-- =========================================================
...
BEGIN
  ...
  ELSE -- open_tasks (Próximas Ações) — REGRA NOVA
    WITH f AS (
      SELECT t.id, t.task_type, t.client, t.clientcode, t.name,
             t.start_date, t.end_date, t.next_action, t.next_action_date
      FROM public.tasks t
...
## 2. Contagem estimada com a nova regra (base atual, 25/08/2026)

| Bucket | Itens | Usuários distintos |
|---|---|---|
| Atrasada (`< hoje`) | 191 | 33 |
| Hoje (`= hoje`) | 11 | 5 |
| Próxima (`> hoje` e `<= hoje+7`) | 7 | 5 |
| **Total no painel** | **209** | — |
| Fora da janela (`> hoje+7`, não exibido) | 11 | 5 |

Comparativo: a regra antiga (`status = 'pending'`) traria ~13.244 registros. Redução de ~98,4%.
Cada usuário vê apenas a sua fatia (filtro `created_by = auth.uid()`).

## 3. Sem duplicidade entre buckets

`next_action_date` é uma única data por tarefa e o `CASE` é sequencial e mutuamente exclusivo
(`< hoje` → `= hoje` → `<= hoje+7` → `NULL`). Uma tarefa cai em exatamente um bucket, ou em nenhum.
`get_my_day_details` usa as mesmas três condições, também disjuntas, garantindo aderência 1:1 com os contadores do resumo.

## 4. Blocos não alterados

Metas (`activity_goal_settings`), visitas realizadas, ligações/prospecções realizadas, programação de visitas, retornos, treinamentos, `my_day_context()`, validações de acesso, `SECURITY DEFINER`, REVOKE/GRANT, chaves do JSON e paginação: idênticos ao SQL aprovado. A chave `open_tasks` é mantida no JSON; apenas os itens ganham `next_action` (texto da ação) além dos campos já previstos.

## 5. Nada de dados/estrutura é alterado

Somente `CREATE OR REPLACE FUNCTION` e GRANT/REVOKE. Nenhum DDL de tabela, nenhum DML, nenhuma política de RLS, nenhum índice, nenhum enum.

# M2 — Migration completa para revisão (NÃO executada)

Escopo: funções-base + RLS já mapeadas. `my_day_scope()` original permanece intacta; é criada `my_day_scope_v2()`. Nenhuma RPC da M3, nenhum frontend, nenhum vínculo real para o Diogo. `get_supervisor_filial_id` e `get_user_filial_id` seguem representando apenas a filial principal. Admin/manager são avaliados primeiro em todos os ramos, sem alteração. Usuário sem vínculo adicional: `get_user_filial_ids` devolve exatamente `[filial principal]`, portanto `user_can_access_filial(x)` equivale a `x = filial principal` — comportamento idêntico ao atual.

## SQL completo

```sql
-- =====================================================================
-- M2 — Escopo multi-filial: funções-base + RLS mapeadas
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1. Helper interno (sem checagem de chamador; não exposto a authenticated)
-- ---------------------------------------------------------------------
...
-- sem GRANT para authenticated: uso exclusivo por funções SECURITY DEFINER

-- ---------------------------------------------------------------------
-- 2. user_same_filial — igualdade -> interseção de conjuntos
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 3. pops_scope — acrescenta filial_ids, mantém scope e filial_id
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 4. my_day_scope_v2 — NOVA. my_day_scope() original NÃO é tocada.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 5. can_insert_vacation — filial principal -> filiais permitidas
-- ---------------------------------------------------------------------
...
-- campaign_clients_delete: não contém filial, mantida como está.

-- 6.2 special_conditions ----------------------------------------------
...
-- 6.4 task_followups (mantém o padrão (SELECT ...) por performance) ---
...
-- trainings_insert_scope: não usa filial, mantida.
...
## Não alterado nesta migration

- `my_day_scope()` original, `get_supervisor_filial_id`, `get_user_filial_id`, `has_role` e todo o RBAC.
- Todas as RPCs da M3 (métricas, gestão, tarefas, clientes, CRM, POPS, Meu Dia, Regularização).
- `equipment_regularization_batches/items`, `pops_import_rows`, Parque de Máquinas, `tasks`, `profiles`, `task_equipment`, `task_access_metadata`, `can_view_opportunity`.
- Nenhum dado: nenhum INSERT/UPDATE em `user_filiais` ou `profiles`; o Diogo continua somente com Caiapônia.
- Nenhum arquivo de frontend.

## Bateria de testes após a aplicação (em `BEGIN … ROLLBACK`)

1. Baseline por role (1 filial): contagens de `campaign_clients`, `special_conditions`, `visit_schedules`, `task_followups`, `trainings`, `clients`, `opportunities`, `pops_machines` iguais às de antes da M2.
2. Vínculo adicional criado apenas dentro da transação: usuário passa a ver as duas filiais.
3. Terceira filial: `user_can_access_filial` false, SELECT vazio, INSERT recusado.
4. Admin/manager: contagens globais idênticas, indiferentes a `user_filiais`.
5. Tentativa de forçar `filial_id` alheio em insert/update: violação de RLS.
6. `user_same_filial`, `pops_scope`, `my_day_scope_v2`: interseção correta, `scope` inalterado, `filial_id` = principal.
7. Desativação do vínculo dentro da transação: acesso cessa na consulta seguinte.
8. `EXPLAIN ANALYZE` das listagens principais sem degradação relevante.

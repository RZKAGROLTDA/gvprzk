...
DECLARE
  ...
  v_supervisor_filiais uuid[] := CASE WHEN v_is_supervisor THEN public.get_user_filial_ids_internal(v_uid) ELSE NULL END;
  v_total integer;
  v_rows jsonb;
BEGIN
  ...
  WITH scoped AS (
    ...
    WHERE (p_start_date IS NULL OR tf.activity_date::date >= p_start_date)
      ...
      AND (v_is_manager OR tf.responsible_user_id = v_uid OR (v_is_supervisor AND tf.filial_id = ANY(v_supervisor_filiais)))
  ),
  filtered AS (

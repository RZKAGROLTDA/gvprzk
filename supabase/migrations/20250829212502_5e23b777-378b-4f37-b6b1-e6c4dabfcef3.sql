-- CRITICAL SECURITY FIXES - Phase 1: Business Intelligence Data Protection

-- 1. Enable RLS on vw_oportunidades_kpis view to prevent unauthorized access
ALTER VIEW public.vw_oportunidades_kpis SET (security_barrier = true);

-- 2. Create RLS policy for vw_oportunidades_kpis view with role-based access control
CREATE POLICY "Secure BI data access with role-based masking" 
ON public.vw_oportunidades_kpis 
FOR SELECT 
USING (
  ...
  auth.uid() IS NOT NULL AND (
    ...
    auth.uid() = vw_oportunidades_kpis.vendedor_id OR
    
    -- Consultants can see low-value data from same filial
...
-- 3. Fix Security Definer functions by adding proper search_path
...
-- 4. Fix other Security Definer functions with search_path
...
-- 5. Fix remaining functions missing search_path
...
-- 6. Add comprehensive audit logging for BI data access
...
-- 7. Create trigger to monitor BI view access
CREATE OR REPLACE FUNCTION public.monitor_bi_view_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  -- Log high-value BI data access
  IF COALESCE(NEW.valor_total_oportunidade, 0) > 25000 OR COALESCE(NEW.valor_venda_fechada, 0) > 25000 THEN
    PERFORM secure_log_security_event(
      'high_value_bi_data_access',
      NEW.vendedor_id,
      jsonb_build_object(

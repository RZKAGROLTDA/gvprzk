...
-- 4. Create Customer Data Protection View with Enhanced Masking
...
-- 6. Security Alert Function for High-Risk Events
...
-- 7. Customer Data Access Logging Function (replaces trigger approach)
CREATE OR REPLACE FUNCTION public.log_customer_data_access_enhanced(
  table_name text,
  operation_type text DEFAULT 'view',
  has_sensitive_data boolean DEFAULT false,
  sales_value numeric DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  PERFORM public.secure_log_security_event(
    ...
    jsonb_build_object(
      ...
      'is_high_value', COALESCE(sales_value, 0) > 50000
    ),
    CASE 
      WHEN COALESCE(sales_value, 0) > 100000 THEN 5
      WHEN COALESCE(sales_value, 0) > 50000 THEN 4
      WHEN has_sensitive_data THEN 3
      ELSE 2
    END
  );
END;

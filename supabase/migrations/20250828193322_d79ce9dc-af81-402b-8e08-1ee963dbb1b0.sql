-- Criar nova estrutura de tabelas normalizada para opportunities
...
-- 4. Migrar dados da tabela tasks existente para o novo modelo
...
-- 5. Migrar oportunidades da tabela tasks
...
SELECT 
  ...
  CASE 
    ...
    WHEN old_t.sales_confirmed = true AND old_t.sales_type = 'parcial' THEN COALESCE(old_t.partial_sales_value, 0)
    ELSE 0
...
RETURNS TRIGGER AS $$
BEGIN
  -- Atualizar valor_total_oportunidade e valor_venda_fechada
...
BEGIN
  ...
  IF NEW.status != OLD.status THEN
    IF NEW.status = 'Venda Total' THEN
      -- Venda Total: qtd_vendida = qtd_ofertada para todos os itens
      UPDATE public.opportunity_items 
      SET qtd_vendida = qtd_ofertada 
      WHERE opportunity_id = NEW.id;
    ELSIF NEW.status = 'Venda Perdida' THEN
      -- Venda Perdida: qtd_vendida = 0 para todos os itens
      UPDATE public.opportunity_items 
      SET qtd_vendida = 0 
      WHERE opportunity_id = NEW.id;
    END IF;
...
-- 13. Criar políticas RLS para tasks_new
...
-- 14. Criar políticas RLS para opportunities
...
CREATE POLICY "Opportunities: Update own or manager" ON public.opportunities
  FOR UPDATE USING (
    -- Consultant pode editar próprias, exceto reabrir vendas fechadas
    (task_id IN (SELECT id FROM public.tasks_new WHERE vendedor_id = auth.uid()) 
     AND NOT (OLD.status IN ('Venda Total', 'Venda Perdida') AND NEW.status = 'Prospect'))
    OR
    -- Manager pode editar todas
    EXISTS (SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role = 'manager')
  );

-- 15. Criar políticas RLS para opportunity_items
CREATE POLICY "Opportunity Items: Access based on opportunity access" ON public.opportunity_items
  FOR SELECT USING (
    opportunity_id IN (
...
CREATE POLICY "Opportunity Items: Manage based on opportunity access" ON public.opportunity_items
  FOR ALL USING (
    opportunity_id IN (
      SELECT o.id FROM public.opportunities o
      JOIN public.tasks_new t ON o.task_id = t.id
...
-- 16. Criar função para get tasks com opportunities
CREATE OR REPLACE FUNCTION public.get_tasks_with_opportunities()
RETURNS TABLE(
  id UUID,
  tipo TEXT,
  vendedor_id UUID,
...
AS $$
  SELECT 
    ...
    END as conversao_pct
  FROM public.tasks_new t
  LEFT JOIN public.opportunities o ON t.id = o.task_id
  WHERE 

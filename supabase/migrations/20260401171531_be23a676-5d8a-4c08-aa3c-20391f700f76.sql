...
BEGIN
  ...
  WHERE
    (
      v_user_role IN ('admin', 'manager')
      OR (LOWER(TRIM(v_user_role)) = 'supervisor' AND LOWER(TRIM(COALESCE(t.filial, ''))) = LOWER(TRIM(COALESCE(v_user_filial_name, ''))))
      OR t.created_by = v_user_id
    )
    AND (p_start_date IS NULL OR t.created_at >= p_start_date::timestamptz)
    AND (p_end_date IS NULL OR t.created_at <= p_end_date::timestamptz)
    AND (p_filial IS NULL OR t.filial = p_filial)
    AND (p_seller_role IS NULL OR p.role = p_seller_role)
    AND (p_seller_id IS NULL OR t.created_by = p_seller_id::uuid)

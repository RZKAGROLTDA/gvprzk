...
CREATE OR REPLACE FUNCTION public.get_secure_tasks_paginated(p_limit integer DEFAULT 200, p_offset integer DEFAULT 0)
 RETURNS TABLE(
   id uuid, name text, client text, clientcode text, property text, filial text, filial_atendida text,
   email text, phone text, responsible text,
   start_date text, end_date text, start_time text, end_time text,
   status text, priority text, task_type text, observations text,
   is_prospect boolean, sales_type text, sales_value numeric, partial_sales_value numeric, sales_confirmed boolean,
   equipment_quantity integer, family_product text,
   check_in_location jsonb, initial_km integer, final_km integer, propertyhectares numeric, prospect_notes text,
   technical_visit_data jsonb, technical_funnel_stage text,
...
BEGIN
  ...
  IF v_user_role IN ('admin', 'manager') THEN
    ...
    SELECT
      ...
      t.technical_visit_data, t.technical_funnel_stage,
  ...
  IF v_user_role = 'supervisor' AND v_user_filial_name IS NOT NULL THEN
    ...
    FROM tasks t
    WHERE t.filial = v_user_filial_name AND t.created_by != v_user_id

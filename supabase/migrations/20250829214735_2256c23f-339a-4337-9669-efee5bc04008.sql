-- CRITICAL SECURITY FIXES - Phase 1 (Final): Business Intelligence Data Protection

-- 1. Drop existing function first to fix parameter defaults issue
DROP FUNCTION IF EXISTS public.secure_log_security_event(text, uuid, jsonb, integer);

-- 2. Enhance RLS on opportunities table 
...
-- 3. Recreate secure logging function with proper search_path
...
-- 4. Fix Security Definer functions with search_path
...
-- 5. Fix other critical Security Definer functions with search_path
...
-- 6. Fix remaining functions missing search_path
...
-- 7. Fix check_suspicious_login_pattern function with search_path

...
-- UMA ÚNICA TRANSAÇÃO. Qualquer RAISE antes do COMMIT => ROLLBACK integral.
-- Validações de leitura V10..V16 são executadas APÓS o COMMIT, em sessão autenticada.
-- ============================================================================
...
DECLARE
  ...
  v_filiais uuid[];
  v_target uuid := public.resolve_primary_user_id(p_responsible_user_id);
BEGIN
  ...
  WITH scoped AS (
    ...
    WHERE (p_start_date IS NULL OR tf.activity_date::date >= p_start_date)
      ...
      AND (v_target IS NULL OR public.resolve_primary_user_id(tf.responsible_user_id) = v_target)
...
AS $function$
  WITH scope AS (
    ...
    SELECT public.resolve_primary_user_id(ce.validated_by) AS uid,
...
DECLARE
  ...
  v_exec   uuid  := public.resolve_primary_user_id(p_executed_by);
  v_wk     date;
  v_mo     date;
  v_total  integer;
  v_rows   jsonb;
BEGIN
  ...
  WITH base AS (
    SELECT public.resolve_primary_user_id(m.executed_by) AS executed_by,
...
DECLARE
  ...
  v_seller uuid := public.resolve_primary_user_id(p_seller_id);
BEGIN
  ...
  WITH ops AS (
    SELECT
      public.resolve_primary_user_id(tf.responsible_user_id) AS seller_id,
      tf.filial_id,
...
BEGIN
  ...
  comm_tasks AS (
    SELECT DISTINCT
      ...
      public.resolve_primary_user_id(t.created_by) AS seller_id,
      t.id AS task_id
    FROM tasks t
...
BEGIN
  ...
  chk AS (
    SELECT
      ...
      pp.filial_id,
      public.resolve_primary_user_id(t.created_by) AS created_by,
...
BEGIN
  ...
  WITH approved_sellers AS (
    ...
    WHERE p.approval_status = 'approved'
      ...
      AND NOT EXISTS (SELECT 1 FROM user_account_links l WHERE l.alias_user_id = p.user_id)
...
BEGIN
  ...
  -- V17) Filial Ativa preservada (independente de sessão).

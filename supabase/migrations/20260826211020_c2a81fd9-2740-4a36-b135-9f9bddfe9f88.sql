...
BEGIN
  ...
  ELSIF p_action = 'confirmar' THEN
    IF v_row.matched_equipment_id IS NULL THEN
      RAISE EXCEPTION 'Linha sem vinculo no Parque; use vincular_manual';
    END IF;
    UPDATE public.pops_import_rows
  ...
  ELSIF p_action = 'vincular_manual' THEN
    ...
    UPDATE public.pops_import_rows r
       SET matched_equipment_id = p_equipment_id,
           ...
           match_score=99,
           match_reason='Vinculacao manual pela gestao',
           candidates='[]'::jsonb,
...
BEGIN
  IF NOT public.pops_is_manager() THEN
    RAISE EXCEPTION 'Acesso negado' USING ERRCODE = '42501';
  END IF;
  IF v_norm IS NULL THEN RAISE EXCEPTION 'Codigo de cliente invalido'; END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Usuario alvo nao e um RAC aprovado e ativo';
  END IF;
...
BEGIN
  ...
  IF jsonb_array_length(v_alvos) = 0 THEN
    RAISE EXCEPTION 'Nenhuma maquina POPS valida na selecao';
  END IF;
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
   ...
   WHERE ca.rac_user_id <> p_rac_user_id;

  IF jsonb_array_length(v_conflitos) > 0 AND NOT p_force THEN
    RAISE EXCEPTION 'Divergencia com o RAC padrao do cliente. Use p_force=true para acao explicita da gestao. Conflitos: %', v_conflitos;
  END IF;
  ...
  SELECT count(*)::integer INTO v_criadas FROM ins;
  ...
  RETURN jsonb_build_object(
    ...
    'clientes_padrao_criados', v_criadas,
    ...
    SELECT r.pops_client_code_norm,
     ...
     WHERE m.program_id = p_program_id AND m.active
       ...
       AND CASE (v_scope->>'scope')
             ...
             WHEN 'filial' THEN e.filial_id = (v_scope->>'filial_id')::uuid
             ELSE false
           END

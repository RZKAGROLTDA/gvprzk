...
CREATE OR REPLACE FUNCTION public.get_weekly_followups_agenda(
  p_start_date date,
  p_end_date date,
  p_responsible_user_id uuid DEFAULT NULL::uuid,
  p_filial_id uuid DEFAULT NULL::uuid
)
RETURNS TABLE(day date, total_activities bigint, visitas bigint, ligacoes bigint, checklists bigint, unique_clients bigint)
LANGUAGE sql
STABLE
...
AS $function$
  ...
  filtered AS (
    SELECT
      f.*,
      (f.activity_date)::date AS activity_day
    FROM public.task_followups f
    WHERE (f.activity_date)::date >= p_start_date
      AND (f.activity_date)::date <= p_end_date
  ...
  FROM days d
  LEFT JOIN filtered f ON f.activity_day = d.day
  GROUP BY d.day
  ORDER BY d.day;
$function$;

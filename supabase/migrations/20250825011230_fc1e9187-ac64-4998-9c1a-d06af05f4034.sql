...
-- First drop the existing policy and create new enhanced ones
DROP POLICY IF EXISTS "Users can view all tasks they have access to" ON public.tasks;

CREATE POLICY "Enhanced role-based task access" 
ON public.tasks 
FOR SELECT 
USING (
  -- Own tasks
  ...
  -- Supervisors can see tasks in their filial only
  (EXISTS (
  ...
  -- Other role consultants can only see same filial tasks with restrictions
  (EXISTS (
    SELECT 1 FROM public.profiles p1, public.profiles p2
    WHERE p1.user_id = auth.uid() 
    ...
    AND p1.filial_id = p2.filial_id
    AND p1.filial_id IS NOT NULL
    -- Limit to lower value tasks for non-supervisory roles
    AND COALESCE(sales_value, 0) <= 25000
  ))
);
...
CREATE POLICY "Enhanced role-based task update control" 
ON public.tasks 
FOR UPDATE 
USING (
  -- Own tasks (with value restrictions for non-managers)
  (auth.uid() = created_by AND (
    NOT is_high_value_task(sales_value) OR 
...
-- SECURITY ENHANCEMENT: Function to log sensitive data access attempts
...
-- SECURITY ENHANCEMENT: Function to monitor email directory access

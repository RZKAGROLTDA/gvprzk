...
serve(async (req) => {
  ...
  try {
    ...
    const { profileId, authUserId, email, cleanupOrphanAuthUser } = body;

    if (cleanupOrphanAuthUser === true) {
      if (!authUserId || !email) {
        return new Response(
          JSON.stringify({ error: 'authUserId and email are required for orphan cleanup' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }

      const { data: authUserData, error: authLookupError } = await supabaseAdmin.auth.admin.getUserById(authUserId);
      ...
      if (existingDependencies.length > 0) {
        return new Response(
          JSON.stringify({
            error: 'Auth user has application dependencies and cannot be deleted safely',

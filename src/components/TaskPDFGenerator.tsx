...
export const generateTaskPDF = async (
  ...
  if (isChecklistPDF) {
    // --- Identificação da Máquina com Chassi em destaque ---
    sectionTitle('Identificação da Máquina');
    ensureSpace(18);
    // Bloco de destaque para chassi
    pdf.setFillColor(240, 245, 255);
    pdf.setDrawColor(PRIMARY[0], PRIMARY[1], PRIMARY[2]);
    pdf.setLineWidth(0.4);
    pdf.roundedRect(marginLeft, yPos - 2, contentWidth, 12, 2, 2, 'FD');
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7);
    pdf.setTextColor(PRIMARY[0], PRIMARY[1], PRIMARY[2]);
    pdf.text('CHASSI / Nº DE SÉRIE', marginLeft + 3, yPos + 2);
    pdf.setFontSize(13);
    pdf.setTextColor(0, 0, 0);
    pdf.text(String(machinePDF.chassi_serie || 'Não informado'), marginLeft + 3, yPos + 8);
    pdf.setLineWidth(0.2);
    yPos += 14;

    twoColRow('Tipo', String(machinePDF.tipo || '—'), 'Modelo', String(machinePDF.modelo || '—'));
    twoColRow('Ano', String(machinePDF.ano || '—'), 'Horímetro', String(machinePDF.horimetro || '—'));
    twoColRow('Status', String(machinePDF.status || '—'), '', '');
    if (machinePDF.observacao) {
      ensureSpace(6);
      pdf.setFont('helvetica', 'italic');
      pdf.setFontSize(8);
      pdf.setTextColor(MUTED[0], MUTED[1], MUTED[2]);
      const lines = pdf.splitTextToSize(`Observação da máquina: ${String(machinePDF.observacao)}`, contentWidth);
    ...
    sectionTitle('Resultado do Checklist');
    // Conclusão automática em caixa colorida
    {
      const boxColor: [number, number, number] =
        cCountPDF.naoConforme > 0 ? [253, 232, 232]
        : cCountPDF.atencao > 0 ? [255, 247, 230]
        : [232, 250, 238];
      const borderColor: [number, number, number] =
        cCountPDF.naoConforme > 0 ? DANGER
        : cCountPDF.atencao > 0 ? WARNING
        : SUCCESS;
      ensureSpace(10);
      pdf.setFillColor(boxColor[0], boxColor[1], boxColor[2]);
      pdf.setDrawColor(borderColor[0], borderColor[1], borderColor[2]);
      const lines = pdf.splitTextToSize(checklistConclusionPDF, contentWidth - 6);
      const boxH = lines.length * 4.6 + 5;
      pdf.roundedRect(marginLeft, yPos - 3, contentWidth, boxH, 2, 2, 'FD');
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(9);
    ...
    if (checklistItemsPDF.length > 0) {
      ...
      pdf.text('Item', marginLeft + 8, yPos);
      pdf.text('Resultado', marginLeft + 90, yPos);
      pdf.text('Observação', marginLeft + 125, yPos);
      pdf.setTextColor(0, 0, 0);
      yPos += 5;

      const statusMeta: Record<string, { sym: string; label: string; color: [number, number, number] }> = {
        conforme: { sym: '[v]', label: 'Conforme', color: SUCCESS },
        atencao: { sym: '[!]', label: 'Atenção', color: WARNING },
        nao_conforme: { sym: '[x]', label: 'Nao conforme', color: DANGER },
        na: { sym: '[-]', label: 'N/A', color: MUTED },
      };

      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(8);
      checklistItemsPDF.forEach((item: any, idx) => {
        const meta = item.responseStatus ? statusMeta[item.responseStatus] : null;
        const note = String(item.responseNotes || item.observations || '');
        const noteLines = note ? pdf.splitTextToSize(note, contentWidth - 127) : ['—'];
        const rowH = Math.max(5, noteLines.length * 4);

        ensureSpace(rowH + 1);
        if (idx % 2 === 1) {
          pdf.setFillColor(247, 249, 252);
          pdf.rect(marginLeft, yPos - 4, contentWidth, rowH, 'F');
        }
        pdf.setTextColor(MUTED[0], MUTED[1], MUTED[2]);
        pdf.text(String(idx + 1), marginLeft + 2, yPos);
        pdf.setTextColor(0, 0, 0);
        const name = pdf.splitTextToSize(item.name || '—', 80);
        pdf.text(name[0], marginLeft + 8, yPos);
        if (meta) {
          pdf.setTextColor(meta.color[0], meta.color[1], meta.color[2]);
          pdf.setFont('helvetica', 'bold');
          pdf.text(`${meta.sym} ${meta.label}`, marginLeft + 90, yPos);
          pdf.setFont('helvetica', 'normal');
          pdf.setTextColor(0, 0, 0);
        } else {
          pdf.setTextColor(MUTED[0], MUTED[1], MUTED[2]);
          pdf.text('—', marginLeft + 90, yPos);
          pdf.setTextColor(0, 0, 0);
        }
        noteLines.forEach((ln: string, li: number) => {
          pdf.text(ln, marginLeft + 125, yPos + li * 4);
        });
        yPos += rowH;
      });
      yPos += 3;

      // --- Fotos por item (se houver) ---
      const itemsWithPhotos = checklistItemsPDF.filter((i: any) => Array.isArray(i.photos) && i.photos.length > 0);
      if (itemsWithPhotos.length > 0) {
        pdf.addPage(); yPos = 20;
        sectionTitle('Fotos por Item');
        for (const item of itemsWithPhotos) {
          const photos: string[] = item.photos.slice(0, 4);
          ensureSpace(50);
          pdf.setFont('helvetica', 'bold');
          pdf.setFontSize(9);
          pdf.text(String(item.name || 'Item'), marginLeft, yPos);
          yPos += 4;
          const loaded = await Promise.all(photos.map(p => loadImageAsBase64(p)));
          ...
          for (let i = 0; i < loaded.length; i++) {
            ...
            if (b64) {
              try {
                ...
                pdf.addImage(b64, 'JPEG', cx, yPos, fw, fh);
              } catch { /* skip */ }
            }
            cx += imgW + gap;
          }
      ...
      if (recommendationsPDF.length === 0) {
        ensureSpace(8);
        ...
        pdf.setFontSize(9);
        pdf.setTextColor(22, 101, 52);
        pdf.text('Nenhuma recomendação técnica registrada.', marginLeft + 3, yPos);
        pdf.setTextColor(0, 0, 0);
        yPos += 8;
      } else {
        recommendationsPDF.forEach((r) => {
          const isCritical = r.status === 'nao_conforme';
          const noteLines = r.note ? pdf.splitTextToSize(r.note, contentWidth - 6) : [];
          const rowH = 6 + noteLines.length * 4 + 2;
          ensureSpace(rowH);
          const bg: [number, number, number] = isCritical ? [253, 232, 232] : [255, 247, 230];
          const bd: [number, number, number] = isCritical ? DANGER : WARNING;
          pdf.setFillColor(bg[0], bg[1], bg[2]);
          pdf.setDrawColor(bd[0], bd[1], bd[2]);
          pdf.roundedRect(marginLeft, yPos - 3, contentWidth, rowH, 1.5, 1.5, 'FD');
          pdf.setFont('helvetica', 'bold');
          pdf.setFontSize(9);
          pdf.setTextColor(bd[0], bd[1], bd[2]);
          pdf.text(`${isCritical ? '[X] NAO CONFORME' : '[!] ATENCAO'}  ·  ${r.name}`, marginLeft + 3, yPos + 1);
          pdf.setFont('helvetica', 'normal');
          pdf.setFontSize(8);
          pdf.setTextColor(60, 60, 60);
          noteLines.forEach((ln: string, li: number) => {
            pdf.text(ln, marginLeft + 3, yPos + 5 + li * 4);
          });
          pdf.setTextColor(0, 0, 0);
          yPos += rowH + 1;
        });
